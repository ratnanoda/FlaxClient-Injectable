    ...
    if (resource != nullptr) {
        ...
        if (resource_bytes != nullptr && resource_size > 0) {
            ...
            if (output && std::filesystem::is_regular_file(target) &&
                ...
                log_message(
                    "Extracted the embedded Minecraft 1.8.9 client jar to %s.",
...
bool clear_java_exception(JNIEnv* env, const char* operation) {
    ...
    if (exception != nullptr) {
        ...
        jmethodID to_string = exception_class == nullptr
                                  ? nullptr
                                  : env->GetMethodID(
                                        exception_class,
                                        "toString",
        ...
        if (to_string != nullptr && !env->ExceptionCheck()) {
            ...
            if (!env->ExceptionCheck() && description != nullptr) {
                const char* utf8 = env->GetStringUTFChars(description, nullptr);
                if (utf8 != nullptr) {
                    detail = utf8;
    ...
    if (detail.empty()) {
        ...
        log_message("Java exception while %s: %s", operation, detail.c_str());
    }
    return true;
}
...
jobject create_jar_url(JNIEnv* env, const std::filesystem::path& jar_path) {
    ...
    jmethodID to_url = uri_class == nullptr
                           ? nullptr
                           : env->GetMethodID(uri_class, "toURL", "()Ljava/net/URL;");
    jobject url = to_url == nullptr ? nullptr : env->CallObjectMethod(uri, to_url);
    ...
    if (uri_class != nullptr) env->DeleteLocalRef(uri_class);
    return url;
}
...
jobject find_loaded_minecraft_class_loader(JavaVM* vm) {
    ...
    for (jint index = 0; index < class_count; ++index) {
        ...
        if (jvmti->GetClassLoader(classes[index], &loader) == JVMTI_ERROR_NONE &&
            loader != nullptr) {
            result = loader;
        }
        break;
    ...
    if (result != nullptr) {
        log_message("Using the defining class loader of the loaded Minecraft 1.8.9 class.");
    }
    return result;
}

jobject find_minecraft_class_loader(JNIEnv* env, JavaVM* vm) {
    jobject loaded_loader = find_loaded_minecraft_class_loader(vm);
    ...
    if (launch_class != nullptr && !env->ExceptionCheck()) {
        ...
        if (class_loader_field != nullptr && !env->ExceptionCheck()) {
            ...
            if (!clear_java_exception(env, "reading Launch.classLoader") &&
                ...
                log_message("Using the Minecraft 1.8.9 LaunchClassLoader.");
                return class_loader;
            }
        }
    ...
    if (class_loader != nullptr) {
        log_message("Using the Java system class loader for Minecraft 1.8.9.");
    }
    return class_loader;
}
...
bool add_jar_to_launch_loader(
    ...
    if (add_url != nullptr &&
        ...
        const bool failed = clear_java_exception(env, "adding the FlaxClient jar");
        if (!failed) {
    ...
    // JVMTI keeps the 1.8.9 client jar visible to Lunar's Java 17 loader.
    jvmtiEnv* jvmti = nullptr;
    if (vm->GetEnv(
            reinterpret_cast<void**>(&jvmti),
            JVMTI_VERSION_1_2) != JNI_OK ||
...
jclass load_class_from(
    ...
    jobject class_object = class_name == nullptr
                               ? nullptr
...
bool mark_late_load_ready(JNIEnv* env, jobject class_loader) {
    ...
    if (mark_core_ready == nullptr || mark_transformer_ready == nullptr ||
        clear_java_exception(env, "resolving LateLoadStatus methods")) {
        env->DeleteLocalRef(status_class);
        return false;
    }

    env->CallStaticVoidMethod(status_class, mark_core_ready);
    bool failed = clear_java_exception(env, "marking the core late-load hook ready");
    if (!failed) {
...
bool retransform_loaded_minecraft_classes(jvmtiEnv* jvmti) {
    ...
    if (found == 0) {
        log_message("Minecraft 1.8.9 hook targets are not loaded yet; hooks will apply on load.");
        return true;
    }

    log_message(
        "Retransformed %d of %d currently loaded Minecraft 1.8.9 hook targets.",
        transformed,
        found);
...
bool install_late_transformer(
    ...
    jclass transformer_class = load_class_from(
        ...
        "loading the FlaxClient 1.8.9 late transformer");
    ...
    log_message("Minecraft 1.8.9 JVMTI late-load transformer installed.");
    return true;
}
...
bool invoke_bootstrap(JNIEnv* env, jobject class_loader) {
    jclass bootstrap_class = load_class_from(
        ...
        "loading the FlaxClient 1.8.9 attach bootstrap");
    ...
    const bool failed =
        clear_java_exception(env, "starting the FlaxClient attach bootstrap");
    env->DeleteLocalRef(bootstrap_class);
    return !failed;
}

DWORD WINAPI attach_worker(void*) {
    log_message("FlaxClient 1.8.9 DLL loaded; waiting for the Java VM.");
    ...
    log_message(
        success ? "FlaxClient 1.8.9 Java bootstrap loaded successfully."
                : "FlaxClient 1.8.9 Java bootstrap failed.");
