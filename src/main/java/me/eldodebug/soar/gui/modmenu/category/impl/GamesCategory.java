...
public class GamesCategory extends Category {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        for(GameScene scene : scenes) {
            boolean hovered = MouseUtils.isInside(mouseX, mouseY,
                    this.getX() + 15, this.getY() + offsetY, this.getWidth() - 30, 40);
            nvg.drawRoundedRect(this.getX() + 15, this.getY() + offsetY,
                    this.getWidth() - 30, 40, 8,
                    palette.getBackgroundColor(hovered ? ColorType.NORMAL : ColorType.DARK));
            nvg.drawText(scene.getIcon(), this.getX() + 26, this.getY() + offsetY + 13F,
                    palette.getFontColor(ColorType.DARK), 14, Fonts.LEGACYICON);
            nvg.drawText(scene.getName(), this.getX() + 47, this.getY() + offsetY + 9F,
                    palette.getFontColor(ColorType.DARK), 12.5F, Fonts.MEDIUM);
            nvg.drawText(scene.getDescription(), this.getX() + 47, this.getY() + offsetY + 23,
                    palette.getFontColor(ColorType.NORMAL), 7.5F, Fonts.REGULAR);
            nvg.drawText(LegacyIcon.CHEVRON_RIGHT,
                    this.getX() + this.getWidth() - 32, this.getY() + offsetY + 15,
                    palette.getFontColor(ColorType.NORMAL), 10, Fonts.LEGACYICON);
            offsetY += 50;
        }
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        ...
        for(GameScene scene : scenes) {
            if(MouseUtils.isInside(mouseX, mouseY, this.getX() + 15,
        ...
        if(!MouseUtils.isInside(mouseX, mouseY, this.getX(), this.getY(),
                this.getWidth(), this.getHeight()) && mouseButton == 0) {
