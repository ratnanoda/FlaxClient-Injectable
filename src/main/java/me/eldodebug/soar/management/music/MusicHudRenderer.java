...
/** Compact bottom-right player shown while a YouTube item is in Music mode. */
public final class MusicHudRenderer extends HUDMod {

    public MusicHudRenderer() {
        super(TranslateText.YOUTUBE, TranslateText.YOUTUBE_PIP_DESCRIPTION);
        setDraggable(false);
    }
    ...
    public void onRender2D(EventRender2D event) {
        ...
        YouTubeEntry entry = manager == null ? null : manager.getCurrent();
        if(manager == null || entry == null || !manager.isMusicHudVisible()) return;

        ScaledResolution resolution = new ScaledResolution(mc);
        NanoVGManager nvg = Glide.getInstance().getNanoVGManager();
        String title = fitTitle(nvg, entry.getTitle(), 172.0F);
        float width = 206.0F;
        float height = 34.0F;
        ...
        nvg.setupAndDraw(() -> {
            ...
            drawRoundedRect(24.0F, 21.5F, 172.0F, 3.0F, 1.5F,
                    new Color(255, 255, 255, 45));
            drawRoundedRect(24.0F, 21.5F, 172.0F * progress, 3.0F, 1.5F,
                    getFontColor(205));

            String playbackText = manager.isLoading() ? "Downloading..."
                    : manager.isPaused() ? "Paused"
                    : formatTime(manager.getPositionMillis());
            drawText(playbackText, 24.0F, 26.0F, 6.5F, getHudFont(1), getFontColor(145));
            setWidth((int) width);
